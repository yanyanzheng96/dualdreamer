...
def parse_safeloras_embeds(
    ...
    """
    embeds = {}
    metadata = safeloras.metadata() #metadata() only contains Textual Inversion key and embeds

    for key in safeloras.keys():
        # Only handle Textual Inversion embeds
        #print(key) # unet:9:down
        ...
        print(key) # <dance1>, <robot1> ...
        print(embeds[key].shape) # torch.Size([768]), torch.Size([768]), ...
...
def collapse_lora(model, alpha=1.0):
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if isinstance(_child_module, LoraInjectedLinear):
            ...
            # print(_child_module.linear.weight.data.shape)
            ...
            # print(_child_module.conv.weight.data.shape)
...
def monkeypatch_or_replace_lora_extended(
    ...
    module_lst = list(_find_modules(model, target_replace_module,\
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if isinstance(_child_module, nn.Linear) or (
...
def monkeypatch_or_replace_safeloras(models, safeloras):
    ...
    for name, (lora, ranks, target) in loras.items():
        # print(name) # text_encoder, unet
        # print(len(lora)) # text_encoder 96, unet 288
        # print(lora[0].shape) # text_encoder torch.Size([768, 1]), unet torch.Size([320, 1])
        # print(ranks) # [1, 1, 1, 1, 1, 1, 1,
        # print(target) # text_encoder ['CLIPAttention'], unet ['Attention', 'CrossAttention', 'GEGLU']
        # print(name)
