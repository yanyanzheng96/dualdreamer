...
from cam_utils import orbit_camera, OrbitCamera, orbit_camera_flip
...
def icp_known_point_cloud(source_points, target_points, max_iterations=50, tolerance=1e-6):
    """
    Perform ICP to align source points to target points.

    Args:
    - source_points: np.array of shape (N, 3), the source point cloud
    - target_points: np.array of shape (M, 3), the target point cloud
    - max_iterations: int, maximum number of ICP iterations
    - tolerance: float, convergence tolerance

    Returns:
    - transformation: np.array of shape (4, 4), the transformation matrix
    - source_points_transformed: np.array of shape (N, 3), the transformed source points
    """
    ...
    # Perform ICP
    threshold = 1.0  # Distance threshold for point correspondence
    ...
    source_points_transformed = np.asarray(source_pcd.transform(transformation).points)

    return transformation, source_points_transformed, icp_result.fitness



def align_with_fixed_lambda(source_points, target_points, fixed_lambda=1.0, max_iterations=50, tolerance=1e-6):
    """
    Align the source point cloud to the target point cloud using a fixed scale factor (lambda).

    Args:
    - source_points: np.array of shape (N, 3), the source point cloud
    - target_points: np.array of shape (M, 3), the target point cloud
    - fixed_lambda: float, fixed value of the scale factor
    - max_iterations: int, maximum number of ICP iterations
    - tolerance: float, convergence tolerance
    ...
    scaled_source_points = fixed_lambda * source_points

    # Perform ICP with the scaled source points
    transformation, source_points_transformed, _ = icp_known_point_cloud(scaled_source_points, target_points, max_iterations, tolerance)

    return transformation, source_points_transformed

# Example usage:
# pts_coord_world.T is the source point cloud (N, 3)
# xyz is the target point cloud (M, 3)
...
def optimize_lambda(source_points, target_points, initial_lambda=1.0, max_iterations=50, tolerance=1e-6):
    """
    Optimize the scale factor lambda to minimize the alignment error between source and target point clouds.

    Args:
    - source_points: np.array of shape (N, 3), the source point cloud
    - target_points: np.array of shape (M, 3), the target point cloud
    - initial_lambda: float, initial value of the scale factor
    - max_iterations: int, maximum number of ICP iterations
    - tolerance: float, convergence tolerance

    Returns:
    - optimal_lambda: float, the optimized scale factor
    - transformation: np.array of shape (4, 4), the transformation matrix
    - source_points_transformed: np.array of shape (N, 3), the transformed source points
    """
    def icp_error(lambda_scale):
        scaled_source_points = lambda_scale * source_points
        _, _, fitness = icp_known_point_cloud(scaled_source_points, target_points, max_iterations, tolerance)
        return -fitness  # Minimize negative fitness (maximize fitness)

    result = minimize(icp_error, initial_lambda, method='Nelder-Mead', options={'maxiter': 100})

    optimal_lambda = result.x[0]
    scaled_source_points = optimal_lambda * source_points
    transformation, source_points_transformed, _ = icp_known_point_cloud(scaled_source_points, target_points, max_iterations, tolerance)

    return optimal_lambda, transformation, source_points_transformed
...
# Replace the previous apply_transformation function with this one in your script
def apply_transformation(pts, transformation):
    """
    Apply a transformation matrix to a point cloud.

    Args:
    - pts: np.array of shape (N, 3), the point cloud
    - transformation: np.array of shape (4, 4), the transformation matrix

    Returns:
    - np.array of shape (N, 3), the transformed point cloud
    """
    ...
    transformed_pts_h = (transformation @ pts_h.T).T
    
    # Convert back to 3D coordinates
    transformed_pts = transformed_pts_h[:, :3].T
...
class GUI:
    ...
    def train_depth(self, i):
        ...
        if True:
            ...
            # render_frames_list.append(images)   # len 14(self.opt.num_frames),  10*3*512*512 
            for p in range(len(reverse_frames_list)):
                images_pt = reverse_frames_list[p]
                alphas = alphatemp_frames_list[p]
                for c in range( images_pt.shape[0] ):

                    image = images_pt[c,:,:,:]
                    image_curr = image_pt2pil( image )
                    #depth_curr = self.d(image_curr)
                    depth_curr = self.d( image_curr )

                    alpha_mask = alphas[c, 0, :, :].detach().cpu().numpy()  # Shape: 512 by 512
                    alpha_mask = alpha_mask.flatten()>0.2  # Flatten to 1D array

                    # depth_curr = depth_curr * alpha_mask


                    pose = orbit_camera_flip( vers[c], hors[c], radii[c])

                    R0, T0 = pose[:3,:3], pose[:3,3:4]
                    ...
                    save_as_ply(pts_coord_world, pts_colors, f"./test_visual/depth_raw.ply")




                    def rotate_y_axis(pts_coord_world, angle_degrees):
                        """
                        Rotate the point cloud around the y-axis relative to its center.

                        Args:
                        - pts_coord_world: np.array of shape (3, n)
                        - angle_degrees: float, angle to rotate in degrees

                        Returns:
                        - rotated_pts_coord_world: np.array of shape (3, n) with points rotated around the y-axis
                        """
                        ...
                        center = np.mean(pts_coord_world, axis=1).reshape(3, 1)

                        # Translate the point cloud to the origin
                        centered_pts_coord_world = pts_coord_world - center

                        # Apply the rotation
                        rotated_centered_pts_coord_world = np.dot(rotation_matrix, centered_pts_coord_world)

                        # Translate the point cloud back to its original center
                        rotated_pts_coord_world = rotated_centered_pts_coord_world + center

                        return rotated_pts_coord_world


                    pts_coord_world = rotate_y_axis(pts_coord_world, 180)



                    save_as_ply(pts_coord_world, pts_colors, f"./test_visual/depth_rotatey.ply")
            ...
            # Filter the points in pts_coord_world and pts_colors using the mask
            save_as_ply(pts_coord_world[:, alpha_mask], pts_colors[alpha_mask, :], f"./test_visual/depth_filter.ply")


            xyz_ = self.GSlist[b_idx].gaussians._xyz.detach().cpu().numpy()

            # # Run optimization to find the best lambda
            # optimal_lambda, transformation_matrix, aligned_pts_coord_world = optimize_lambda(pts_coord_world[:, alpha_mask].T, xyz_)

            fixed_lambda = 1.0  # Example fixed lambda value
            transformation_matrix, aligned_pts_coord_world = align_with_fixed_lambda(pts_coord_world[:, alpha_mask].T, xyz_, fixed_lambda)


            save_as_ply(aligned_pts_coord_world.T, pts_colors[alpha_mask, :], f"./test_visual/depth_align.ply")

            # Apply the transformation to pts_coord_world_full
            pts_coord_world = apply_transformation(pts_coord_world, transformation_matrix)
            save_as_ply(pts_coord_world, pts_colors, f"./test_visual/depth_align_full.ply")
    ...
    def train_step(self, i):
        ...
        for _ in range(self.train_steps): # 1
            ...
            os.makedirs( step_dir, exist_ok=True )
            for b_idx in range(self.opt.num_frames):
                self.GSlist[b_idx].gaussians.save_ply( os.path.join( step_dir, f'model_{b_idx}.ply' ) )
                export_to_gif(full_list, os.path.join( step_dir, f'train_full_step_{i}_hor_{hor}.gif' ) )
