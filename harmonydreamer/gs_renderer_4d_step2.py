...
class Renderer:
    ...
    def save_plys(
        ...
        for b_idx in range(14):
            ...
            time = torch.tensor(b_idx).to(means3D.device).repeat(means3D.shape[0],1)
            time = (time.float()  - 4) / 10 # hack
            ...
            temp_gaussians = copy.deepcopy( self.gaussians )
            temp_gaussians._xyz = means3D_deform
            temp_gaussians._features_dc = self.gaussians._features_dc
            temp_gaussians._features_rest = self.gaussians._features_rest
            temp_gaussians._opacity = self.gaussians._opacity
            temp_gaussians._scaling = scales_deform
            temp_gaussians._rotation = rotations_deform

            temp_gaussians.save_ply( os.path.join( save_path, f'model_{b_idx}.ply' ) )
