        ...
        for _ in range(self.train_steps): # 1
            ...
            for iii in range(train_iters):
                ...
                if iii % 200 == 0 :
                    ...
                    demo_dir = os.path.join( self.opt.data_dir, f'demo_logs_4D/exp_{self.opt.timestamp}_4D')
                    os.makedirs( demo_dir, exist_ok=True )
                    export_to_gif(front_list, os.path.join(demo_dir, f'train_front_{self.step}_iter_{iii}_hor_{hor}.gif'))
                    export_to_gif(full_list, os.path.join(demo_dir, f'train_full_{self.step}_iter_{iii}_hor_{hor}.gif') )

            step_dir = os.path.join( demo_dir , f'step_{i}')
