...
class StableVideoDiffusion:
    ...
    def get_inverse_loop(
        ...
        # interp to 512x512 to be fed into vae.
        pred_rgb_512 = F.interpolate(pred_rgb, (wid, 512), mode="bilinear", align_corners=False)
        # encode image into latents with vae, requires grad!
        # latents = self.pipe._encode_image(pred_rgb_512, self.device, num_videos_per_prompt=1, do_classifier_free_guidance=True)
        ...
        with torch.no_grad():
            ...
            for b in range(batch_size):
                latents_ = self.encode_image(pred_rgb_512[b:b+1,:,:,:])
        ...
        # inverse  # in this code, denoise_end 25 towards pixel space
        with torch.no_grad():
            #t = self.num_train_timesteps - t.item()
            # add noise
            #noise = torch.randn_like(latents)
            #latents_noisy = self.pipe.scheduler.add_noise(latents, noise, self.pipe.scheduler.timesteps[t:t+1]) # t=0 noise;t=999 clean
            latents = self.pipe.inv(
                image=self.image,
                # image_embeddings=self.embeddings, 
                height=512,
                width=wid,
                latents=latents,
                output_type='latents', 
                denoise_beg=self.num_train_timesteps,
                denoise_end=self.num_train_timesteps - inverse_steps,
                min_guidance_scale=min_guidance_scale,
                max_guidance_scale=max_guidance_scale,
                num_frames=batch_size,
                num_inference_steps=self.num_train_timesteps
        ...
        with torch.no_grad():
            frames = self.pipe(
                ...
                latents=latents,
                output_type='frames', 
                denoise_beg=self.num_train_timesteps - inverse_steps,
                denoise_end=self.num_train_timesteps ,
                min_guidance_scale=min_guidance_scale,
                max_guidance_scale=max_guidance_scale,
                num_frames=batch_size,
                num_inference_steps=self.num_train_timesteps
