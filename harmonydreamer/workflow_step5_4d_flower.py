...
class GUI:
    def __init__(self, opt):
        ...
        os.makedirs('../demo_logs_4D', exist_ok = True)
        ...
        for b_idx in range(self.opt.num_frames):
            ...
            self.renderer.initialize(f'./logs_/robot/model_{b_idx}.ply')  # torch.Size([17290, 3])
            self.renderer_add1.initialize(f'./logs_/fish/model_{b_idx}.ply') # 
            ...
            self.renderer.gaussians.save_ply('../demo_cache/test.ply')



            self.GSlist[b_idx].initialize( os.path.join(self.opt.case_dir, 'plys_4d', f'model_{b_idx}.ply') )

            #self.GSlist[b_idx].initialize(f'./logs_/robot/model_{b_idx}.ply')
    ...
    def train_step(self, i):
        ...
        for _ in range(self.train_steps): # 1
            ...
            for _ in range(self.opt.batch_size):
                ...
                # hor = np.random.randint(-45, 45)     #-45 45
                hor = np.random.randint(-25, 25)     #-45 45
                # radius = 0
            ...
            for _ in range(num_reverse):
                ...
                os.makedirs( f'../demo_logs_4D/exp_{self.opt.timestamp}_4D', exist_ok = True )
                export_to_gif(pngs, f'../demo_logs_4D/exp_{self.opt.timestamp}_4D/inversion_{self.step}_hor_{hors[_]}.gif')
            ...
            for iii in range(train_iters):
                ...
                if iii % 200 == 0 :
                    ...
                    os.makedirs( f'../demo_logs_4D/exp_{self.opt.timestamp}_4D', exist_ok=True )
                    export_to_gif(front_list, f'../demo_logs_4D/exp_{self.opt.timestamp}_4D/train_front_{self.step}_iter_{iii}_hor_{hor}.gif')
                    export_to_gif(full_list, f'../demo_logs_4D/exp_{self.opt.timestamp}_4D/train_full_{self.step}_iter_{iii}_hor_{hor}.gif')

            step_dir = os.path.join( f'../demo_logs_plys/exp_{self.opt.timestamp}_4D' , f'step_{i}')
...
if __name__ == "__main__":
    ...
    os.makedirs( f'../demo_logs_4D/exp_{opt.timestamp}_4D', exist_ok=True )
    current_script_path = os.path.abspath(sys.argv[0])
    destination_path = os.path.join(f'../demo_logs_4D/exp_{opt.timestamp}_4D', os.path.basename(current_script_path))
