    ...
    def get_inverse_loop(
        ...
        with torch.no_grad():
            ...
            latents = self.pipe.inv(
                ...
                latents=latents,
                output_type='noise', 
                denoise_beg=len(self.pipe.scheduler.timesteps)-1,
                denoise_end=len(self.pipe.scheduler.timesteps)-inverse_steps,
                min_guidance_scale=min_guidance_scale,
                max_guidance_scale=max_guidance_scale,
                num_frames=batch_size,
                num_inference_steps=self.num_train_timesteps
        ...
        with torch.no_grad():
            ...
            frames = self.pipe(
                ...
                latents=latents,
                output_type='video', 
                denoise_beg=len(self.pipe.scheduler.timesteps)-inverse_steps + 1,
                denoise_end=len(self.pipe.scheduler.timesteps),
                min_guidance_scale=min_guidance_scale,
                max_guidance_scale=max_guidance_scale,
                num_frames=batch_size,
                num_inference_steps=self.num_train_timesteps
