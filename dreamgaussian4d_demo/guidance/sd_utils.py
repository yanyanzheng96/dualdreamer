...
class StableDiffusion(nn.Module):
    ...
    def get_inverse_loop(self, pred_rgb, prompts, negative_prompts, back_ratio_list=[0.5], guidance_scale=7.5, as_latent=False):
        ...
        with torch.no_grad():
            for i, t in enumerate(timestep_path[0:back_timeindex_list[-1]]):
                ...
                #     input_img_np = input_img_torch_resized.detach().cpu().numpy()
                #     input_img_np = (input_img_np * 255).astype(np.uint8)
                #     Image.fromarray((input_img_np).astype(np.uint8)).save(f'./paper_demo/checkpredictinversion_{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}_{i}_{t}.png')
            ...
            for i, t in enumerate(reversed(timestep_path[0:back_timeindex_list[-1]])):
                ...
                #     input_img_np = input_img_torch_resized.detach().cpu().numpy()
                #     input_img_np = (input_img_np * 255).astype(np.uint8)
                #     Image.fromarray((input_img_np).astype(np.uint8)).save(f'./paper_demo/checkreversion_{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}_{i}_{t}.png')
    ...
    def get_inverse_loop_onepass(self, pred_rgb, pred_rgb_blur,   prompts, negative_prompts, tstep_blur = 15, tstep_fromlastblur = 15, tstep_tonextblur = 10, guidance_scale=7.5, as_latent=False):
        ...
        self.scheduler.alphas_cumprod = self.scheduler.alphas_cumprod.to(self.device)
        with torch.no_grad():
            for i, t in enumerate(timestep_path[0:tstep_blur+1]):
                print('inverse', i,t)
                ####### get inverse coefficient 
                t = torch.tensor(t, device = self.device)
                ...
                latents_predict = ( latents - (1 - alpha_prod_t_back)**(0.5) * noise_pred_cond ) / (alpha_prod_t_back)**(0.5)

                ######## save needed back prediction 
                dict_latents4predict[t[0].item()] = latents_predict
                dict_modeloutput[t[0].item()] = model_output
        ...
        with torch.no_grad():
            # ######## reverse 
            if pred_rgb_blur != None and tstep_fromlastblur != None and tstep_tonextblur != None:
                pred_rgb = pred_rgb.to(self.dtype)
                batch_size = pred_rgb.shape[0]
                if as_latent:
                    latents = F.interpolate(pred_rgb, (32, 32), mode='bilinear', align_corners=False) * 2 - 1
                ...
                alpha_prod_t = self.scheduler.alphas_cumprod[timestep_path[tstep_fromlastblur]]
                latents_predict = latents_grad.detach()
                sigma = 0.6
                latents = latents_predict*(alpha_prod_t)**(0.5) + (1 - alpha_prod_t - sigma**2)**(0.5) * dict_modeloutput[timestep_path[tstep_fromlastblur]] + sigma*torch.randn_like(latents).to(self.device)
                #latents = latents_predict*(alpha_prod_t)**(0.5) + (1 - alpha_prod_t)**(0.5) * torch.randn_like(latents).to(self.device)
            #     latents = torch.randn_like(latents).to(self.device)

                #breakpoint()
                for i, t in enumerate(reversed(timestep_path[tstep_tonextblur : tstep_fromlastblur])):
                    t = torch.tensor(t, device = self.device)
                    print('reverse', i, t)
                    ...
                    model_output = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
                    latents_predict = ( latents - (1 - alpha_prod_t_prev)**(0.5) * model_output ) / (alpha_prod_t_prev)**(0.5)
                    #latents_predict = ( latents - (1 - alpha_prod_t_prev)**(0.5) * noise_pred_cond ) / (alpha_prod_t_prev)**(0.5)


    
            imgs_next_blur = self.decode_latents(latents_predict[:,:,:,:])

            for key in dict_latents4predict.keys():
                dict_imgs4predict[key] = self.decode_latents(dict_latents4predict[key])

            #imgs = self.decode_latents(latents[:,:,:,:])

            os.makedirs('./test_blur', exist_ok=True)
            for idx in range(imgs_next_blur.shape[0]):
                imgs = imgs_next_blur
                imgs_reverse = imgs
                input_img_torch_resized = imgs[idx,:,:,:].permute(1, 2, 0)
                input_img_np = input_img_torch_resized.detach().cpu().numpy()
                input_img_np = (input_img_np * 255).astype(np.uint8)
                Image.fromarray((input_img_np).astype(np.uint8)).save(f'./test_blur/checkblur_{idx}.png')
                    
        #loss = F.mse_loss( latents_grad , torch.zeros_like(latents_grad ).to(self.device))

        return dict_imgs4predict, imgs_next_blur   # 16*3*256*256
