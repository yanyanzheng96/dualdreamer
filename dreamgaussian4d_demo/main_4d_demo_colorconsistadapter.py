...
class GUI:
    def __init__(self, opt):
        ...
        for b_idx in range(self.opt.num_frames):
            ...
            self.renderer.initialize(f'../demo_data/sand_fish_robot/object_0/plys_4d/model_{b_idx}.ply')  # torch.Size([17290, 3])
            self.renderer_add1.initialize(f'../demo_data/sand_fish_robot/object_1/plys_4d/model_{b_idx}.ply') # 
            ...
            self.GSlist[b_idx].initialize('./logs/test.ply')
    ...
    def train_step(self, i):
        ...
        for _ in range(self.train_steps): # 1
            ...
            for _ in range(num_reverse):
                ...
                os.makedirs( f'./logs_4D/exp_{self.opt.timestamp}_4D', exist_ok = True )
                export_to_gif(pngs, f'./logs_4D/exp_{self.opt.timestamp}_4D/inversion_{self.step}_hor_{hors[_]}.gif')
            ...
            for iii in range(401):
                ...
                loss.backward()
                print(f'exp {self.opt.timestamp}, step {self.step}, iter {iii}', loss.item())
                ...
                if iii % 200 == 0 :
                    ...
                    for t in range(nframes):
                        ...
                        if azimuth_shift >= 90:
                            azimuth_shift = 90
                        start_pixel = int((azimuth_shift - (-90)) / (90 - (-90))*512)
                    ...
                    os.makedirs( f'./logs_4D/exp_{self.opt.timestamp}_4D', exist_ok=True )
                    export_to_gif(front_list, f'./logs_4D/exp_{self.opt.timestamp}_4D/train_front_{self.step}_iter_{iii}_hor_{hor}.gif')
                    export_to_gif(full_list, f'./logs_4D/exp_{self.opt.timestamp}_4D/train_full_{self.step}_iter_{iii}_hor_{hor}.gif')

            step_dir = os.path.join( f'./logs_plys/exp_{self.opt.timestamp}_4D' , f'step_{i}')
...
if __name__ == "__main__":
    ...
    opt.save_path = 'robot'
    opt.batch_size = 24
    opt.prompt = 'robot and iron fish'
