...
class GUI:
    def __init__(self, opt):
        ...
        self.opt.input = os.path.join( self.opt.workdir, 'images_generate' )
        png_files = [f for f in os.listdir(self.opt.input) if f.endswith('.png') and not f.endswith('rgba.png')] 
        ...
        self.opt.input = os.path.join( self.opt.input, sorted_files[0] )
    ...
    def train_step(self):
        ...
        new_height = 512
        bg_image_resized = Image.open(  bg_image_path ).resize((512, 512))
        ...
        bg_tensor = TF.to_tensor(bg_image_resized)
        # Ensure the tensor is in the shape [3, 512, 512]
        bg_tensor_512 = bg_tensor[:3, :, :].to("cuda")

        bg_image_path = self.opt.bg_path
        new_height = 512
        bg_image_resized = Image.open(  bg_image_path ).resize((256, 256))
        # Convert the PIL image to a PyTorch tensor
        bg_tensor = TF.to_tensor(bg_image_resized)
        # Ensure the tensor is in the shape [3, 512, 512]
        bg_tensor_256 = bg_tensor[:3, :, :].to("cuda")

        bg_image_path = self.opt.bg_path
        new_height = 512
        bg_image_resized = Image.open(  bg_image_path ).resize((128, 128))
        # Convert the PIL image to a PyTorch tensor
        bg_tensor = TF.to_tensor(bg_image_resized)
        # Ensure the tensor is in the shape [3, 512, 512]
        bg_tensor_128 = bg_tensor[:3, :, :].to("cuda")
        ...
        for _ in range(self.train_steps):
            ...
            for _ in range(self.opt.batch_size):
                ...
                if self.step > 300:
                    if render_resolution == 512:
                        bg_tensor = bg_tensor_512
                    if render_resolution == 256:
                        bg_tensor = bg_tensor_256
                    if render_resolution == 128:
                        bg_tensor = bg_tensor_128
                    
                    image = alpha * image + (1 - alpha) * bg_tensor.unsqueeze(0)
                ...
                # # Assuming out["image"] and out["alpha"] are provided and correctly shaped
                # image_add = out["image"]  # This should be [3, 512, 512]
                # alpha_add = out["alpha"]  # This should be [1, 512, 512]
                # # Expand the alpha tensor to match the foreground/background shape for broadcasting
                # alpha_add = alpha_add.expand_as(image_add)
                # # Blend the images
                # # The formula for blending is: result = alpha * foreground + (1 - alpha) * background
                # image_add = image_add.unsqueeze(0) # [1, 3, H, W] in [0, 1]
                # alpha_add = alpha_add.unsqueeze(0) # [1, 1, H, W] in [0, 1]

    
                # image_add = alpha_add * image_add + (1 - alpha_add) * bg_tensor.unsqueeze(0)

                # images_add.append(image_add)
                # alphas_add.append(alpha_add) # each mask saved in the list is of shape 1, 512, 512
            ...
            #     images_add = torch.cat(images_add, dim=0)
            #     alphas_add = torch.cat(alphas_add, dim=0)

            #     render_views_ = torch.clone(images_add)
            ...
            #     loss_inverse = F.mse_loss(images_add, imgs_reverse.detach().to(torch.float32))
            # else: 
            #     loss_inverse = 0
            ...
            # densify and prune
            if self.renderer.gaussians._xyz.shape[0] < self.opt.max_pts:
    ...
    def train(self, iters=500, ui=False):
        ...
        # path = os.path.join(self.opt.outdir, self.opt.save_path + '_model.ply')
        self.renderer.gaussians.save_ply( os.path.join( self.opt.workdir, 'ply_3d', 'model.ply') )
        ...
        export_to_gif( image_list, os.path.join(self.opt.workdir, 'ply_3d', 'static.gif') )
 
        with open( os.path.join(self.opt.workdir, 'ply_3d', 'points_num.txt') , 'w') as file:
            file.write(str( self.renderer.gaussians._xyz.shape[0] ))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_pts", required=True, type=int, help="path to the workdir")
